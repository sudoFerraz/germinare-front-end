...
module.exports = function(Chart) {
	...
	Chart.controllers.line = Chart.DatasetController.extend({
		...
		draw: function() {
			...
			var meta = me.getMeta();
            var points = meta.data || [];
            //console.log('chart',chart.config.data.datasets[0].subdatas) //RECEBI OS DADOS AQUI
			...
			for (; i < ilen; ++i) {
                //console.log(i)
				points[i].draw(area, subData); //chart.config.data.datasets[0].subdatas[i] PASSAR SUB DATA CORRETO
			}
		},
			...
			if (optionTicks.display) {
				...
				if (helpers.isArray(label)) {
					...
					for (var i = 0; i < lineCount; ++i) {
                        ...
                        context.fillStyle = i%3 != 0 ? '#fff' : '#626364';
...
module.exports = Element.extend({
	...
	draw: function(chartArea, subData) {
		var vm = this._view;
		...
		if (chartArea === undefined || (model.x >= chartArea.left && chartArea.right * errMargin >= model.x && model.y >= chartArea.top && chartArea.bottom * errMargin >= model.y)) {
            ...
            helpers.canvas.drawPoint(ctx, pointStyle, radius, x, y, rotation, subData); 
            //console.log('DRAW POINTS HERE')
		}
	}
});
...
var exports = module.exports = {
    ...
    drawSubDataContainer: function(ctx, x, y, width, height, radius) {
	...
	drawPoint: function(ctx, style, radius, x, y, rotation, subData) {
        ...
        //console.log(subData) PLOTAR AQUI
        ...
        default:
            ...
            const epsilon = 0.0000001, width = 42, height = 15;
            var r, x, y;
            let up = 0, down = 0;

            if(subData.dolar_flag === 1) {
                up++;
                ctx.fillStyle = '#fff';
                ctx.fillText(''+parseFloat(Math.round(subData.dolar_quote * 100) / 100).toFixed(2), 0, 0 - (up * 20));

                ctx.fillStyle = 'rgba(0,0,0,0)'
                ctx.strokeStyle = '#DE792B';
                r = Math.min(15, (height / 2) - epsilon, (width / 2) - epsilon);
                x = 0 - width/2, y = 0 - (up * 20) - height/2;
            } else {
                down++;
                ctx.fillStyle = '#fff';
                ctx.fillText(''+parseFloat(Math.round(subData.dolar_quote * 100) / 100).toFixed(2), 0, 0 + (down * 20));

                ctx.fillStyle = 'rgba(0,0,0,0)'
                ctx.strokeStyle = '#DE792B';
                r = Math.min(15, (height / 2) - epsilon, (width / 2) - epsilon);
                x = 0 - width/2, y = 0 + (down * 20) - height/2;
            }
            ctx.moveTo(x + r, y);
            ctx.lineTo(x + width - r, y);
            ctx.arcTo(x + width, y, x + width, y + r, r);
            ...
            if(subData.basis_flag === 1) {
                up++;
                ctx.fillStyle = '#fff';
                ctx.fillText(''+parseFloat(Math.round(subData.basis_quote * 100) / 100).toFixed(2), 0, 0 - (up * 20));

                ctx.fillStyle = 'rgba(0,0,0,0)'
                ctx.strokeStyle = '#09B2E0';
                r = Math.min(15, (height / 2) - epsilon, (width / 2) - epsilon);
                x = 0 - width/2, y = 0 - (up * 20) - height/2;
            } else {
                down++;
                ctx.fillStyle = '#fff';
                ctx.fillText(''+parseFloat(Math.round(subData.basis_quote * 100) / 100).toFixed(2), 0, 0 + (down * 20));
            ...
            if(subData.cbot_flag === 1) {
                up++;
                ctx.fillStyle = '#fff';
                ctx.fillText(''+parseFloat(Math.round(subData.cbot_quote * 100) / 100).toFixed(2), 0, 0 - (up * 20));

                ctx.fillStyle = 'rgba(0,0,0,0)'
                ctx.strokeStyle = '#917BB5';
                r = Math.min(15, (height / 2) - epsilon, (width / 2) - epsilon);
                x = 0 - width/2, y = 0 - (up * 20) - height/2;
            } else {
                down++;
                ctx.fillStyle = '#fff';
                ctx.fillText(''+parseFloat(Math.round(subData.cbot_quote * 100) / 100).toFixed(2), 0, 0 + (down * 20));
